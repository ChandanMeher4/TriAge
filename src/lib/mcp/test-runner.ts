...
export class TestRunner {
     ...
     */
    async runTestPlan(input: TestPlanInput, onProgress?: (result: TestResult) => void): Promise<TestRunOutput> {
        ...
        if (steps[0].action !== "navigate") {
            ...
            if (onProgress) onProgress(navResult);
        ...
        for (const step of steps) {
            ...
            if (result.status === "failed" && step.action === "navigate") {
                ...
                for (const s of steps) {
                    ...
                    if (onProgress) onProgress(skippedResult);
                }
