...
async def analyze_failure(request: AnalyzeFailureRequest):
    ...
    try:
        ...
        return AnalyzeFailureResponse(
            rca_report=str(result.get("rca_report", "Failed to generate RCA")),
            proposed_fix=str(result.get("proposed_fix", "")),
            confidence=float(result.get("confidence_score", 0.5)),
        )
    except Exception as e:
        failed_count = sum(1 for r in request.test_results if r.get("status") == "failed")
        return AnalyzeFailureResponse(
            rca_report=f"## Error Generating RCA\n\n{failed_count} tests failed. Healer agent encountered an error: {e}",
            proposed_fix="// Please check the AI Engine logs for more details.",
            confidence=0.5,
        )
